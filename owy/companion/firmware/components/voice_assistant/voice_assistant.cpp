...
// Owy: retain a bounded delayed TCP batch even while the I2S task is restarting.
// 16 KiB overflowed a reproduced 700 ms / 22,528 B burst. 32 KiB lives in PSRAM;
// this is capacity, not added prebuffer latency. Host pacing remains 100 ms.
static const size_t SPEAKER_BUFFER_SIZE = 32 * RECEIVE_SIZE;
...
bool VoiceAssistant::allocate_buffers_() {
  ...
  if ((this->speaker_ != nullptr) && (this->speaker_buffer_ == nullptr)) {
    RAMAllocator<uint8_t> speaker_allocator(RAMAllocator<uint8_t>::ALLOC_EXTERNAL);
